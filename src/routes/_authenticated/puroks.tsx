      ...
      <div className="space-y-8 p-6">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {barangays.map((barangay) => (
            <BarangayCard
              ...
              onSelect={() => setSelectedBarangayId(barangay.id)}
            />
          ))}
        </div>
        ...
        {selectedBarangay && (
          <Dialog
            open
            onOpenChange={(open) => {
              if (!open) setSelectedBarangayId(null);
            }}
          >
            <DialogContent className="max-h-[85vh] max-w-3xl overflow-y-auto">
              <DialogHeader>
                <DialogTitle>Puroks in {selectedBarangay.name}</DialogTitle>
                <DialogDescription>
                  {selectedPuroks.length} puroks · Captain:{" "}
...
function BarangayCard({
  ...
  return (
    ...
    >
      ...
      <p className="mt-4 text-xs font-medium text-indigo-600">View puroks</p>
    </button>
  );
}
...
function PurokCard({
  ...
  const stats = useMemo(() => {
    ...
    const householdsById = new Map(
      allHH.map((household) => [household.id, household]),
    );
    const pMembers = allMembers.filter(
      (member) => memberPurokId(member, householdsById) === purok.id,
    );
    return {
      householdsById,
      pMembers,
  ...
  return (
    <div className="group rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:border-slate-300 hover:shadow-md">
      ...
      <PurokMembersDialog
        ...
        members={stats.pMembers}
        households={stats.householdsById}
      />
    </div>
  );
}
