...
export async function parseEntrySheet(file: File): Promise<ImportSummary> {
  ...
  const data = processImportedData(rows);
...
function processImportedData(rows: EntryRow[]): DataSet {
  // We derive the barangay name from the file context. Since the ENTRY sheet
  // does not include a Barangay column, we create one default barangay.
  ...
  const purokMap = new Map<string, { id: number; leaderName: string }>();
  ...
  for (const row of rows) {
    ...
    if (!purokMap.has(code)) {
      purokIdCounter++;
      purokMap.set(code, { id: purokIdCounter, leaderName: "—" });
    }
  ...
  const puroks: Purok[] = Array.from(purokMap.entries()).map(
    ([code, info]) => ({
      id: info.id,
      barangayId: defaultBarangay.id,
      name: code, // use the CODE as the purok name
  ...
  for (const row of rows) {
    ...
    const code = str(row.CODE);
    const purokInfo = code ? purokMap.get(code) : null;
